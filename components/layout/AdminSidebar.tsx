...
const items = [
  ...
  { href: "/admin/news", label: "ข่าว", icon: Newspaper },
  { href: "/admin/departments", label: "แผนกวิชา", icon: MapPinned },
  { href: "/admin/teachers", label: "อาจารย์", icon: UserCog },
  { href: "/admin/events", label: "กิจกรรม", icon: CalendarDays },
  { href: "/admin/gallery", label: "แกลเลอรี่", icon: Images },
  { href: "/admin/students", label: "นักศึกษา", icon: GraduationCap },
  { href: "/admin/staff", label: "บัญชีผู้ใช้", icon: ShieldCheck },
  { href: "/admin/admissions", label: "ใบสมัคร", icon: FileText },
  { href: "/admin/attendance", label: "รายงาน", icon: Users },
  ...
  return (
    ...
    <aside className="hidden w-72 shrink-0 border-r border-slate-200 bg-white lg:block">
      <div className="flex h-20 items-center gap-3 border-b border-slate-200 px-6">
        ...
        <div>
          <p className="text-lg font-black text-slate-950">Admin CMS</p>
          <p className="text-xs font-semibold text-slate-500">วิทยาลัยเทคนิคลำปาง</p>
        </div>
      </div>
          ...
          return (
            <Link
              ...
              className={`flex min-h-12 items-center gap-3 rounded-2xl px-4 text-sm font-bold transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-blue-100 ${
                active
                  ? "bg-blue-800 text-white shadow-lg shadow-blue-900/15"
      ...
      <div className="mx-4 mt-4 rounded-3xl bg-slate-950 p-5 text-white">
        <MapPinned className="h-6 w-6 text-sky-200" />
        <p className="mt-3 text-sm font-black">Geofence</p>
        <p className="mt-2 text-xs leading-5 text-slate-300">ตั้งค่าพิกัดและรัศมีสำหรับเช็คชื่อในหน้าตั้งค่า</p>
      </div>
    </aside>
